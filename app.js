...
async function person(address,worker){const e=el('span','','person');if(address){const icon=el('span');try{icon.innerHTML=await identiconSvg(address,32)}catch{}e.append(icon)}const d=el('div');if(worker)d.append(el('b',worker));const a=el('a',short(address));a.href='#';a.title=address||'';a.onclick=ev=>{ev.preventDefault();if(address)send('open',{path:'/address/'+encodeURIComponent(address)})};const line=el('span','','addr-line');line.append(a);d.append(line);if(address){const c=el('button','⧉ COPY','copy-btn');c.type='button';c.title='Copy the full address';c.onclick=ev=>{ev.preventDefault();ev.stopPropagation();send('copy',{text:address});c.textContent='✓ COPIED';c.classList.add('done');clearTimeout(c._t);c._t=setTimeout(()=>{c.textContent='⧉ COPY';c.classList.remove('done')},1400)};line.append(c)}e.append(d);return e}
function render(){
...
function renderCreate(){
 ...
 $('createBtn').disabled=wallet.cliFound===false;
...
$('walletCreateForm').onsubmit=e=>{e.preventDefault();const f=new FormData(e.target);if(String(f.get('pass')||'')!==String(f.get('pass2')||'')){$('createState').textContent='✗ PASSPHRASES DIFFER';notice('Passphrases do not match.');return}$('walletSeedBox').hidden=true;send('walletCreate',{name:String(f.get('name')||'').trim(),passphrase:String(f.get('pass')||''),card:f.get('card')==='on'});e.target.elements.pass.value='';e.target.elements.pass2.value=''};
