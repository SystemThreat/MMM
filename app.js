...
window.receive=async msg=>{
 ...
 case 'walletSent':{if(msg.relaunch)selectTab('wallet');walletBusy=false;$('walletUnlockBtn').disabled=$('walletSendBtn').disabled=$('walletLockBtn').disabled=$('walletBackupBtn').disabled=$('bpMake').disabled=false;const ids=msg.txids?.length?msg.txids:msg.txid?[msg.txid]:[],n=Math.max(msg.transactions||0,ids.length,1),head=el('div','','wr-head'),rf=msg.refused;
  ...
  head.append(el('b',msg.partial?`⚠ SENT ${ids.length} OF ${n} TRANSACTIONS`:n>1?`SENT ✓ ${n} TRANSACTIONS`:'SENT ✓'));if(msg.amount)head.append(el('span',' · '+msg.amount+' XID'));if(msg.fee)head.append(el('span',' · fee '+msg.fee+' XID · '+msg.vsize+' vB'));
...
async function renderBlocks(){if($('view-blocks').hidden)return;const revision=++blockRevision;const {start,count}=pageInfo('blocks',blockData.length);const rows=await Promise.all(blockData.slice(start,start+count).map(async b=>{const tr=el('tr'),height=el('td'),a=el('a','#'+fmt(b.height));a.href='#';a.onclick=e=>{e.preventDefault();send('open',{path:'/block/'+b.height})};height.append(a);const payouts=(b.tx?.[0]?.vout||[]).filter(o=>o.value>0);const who=el('td');if(payouts.length){const sp=payouts[0].scriptPubKey||{};who.append(await person(sp.address||sp.addresses?.[0]||decodeScript(sp.hex,profile.network==='mainnet'?'xpa':'txa'),payouts.length>1?`+ ${payouts.length-1} other payout outputs`:''))}else who.textContent='No spendable payout';tr.append(height,who,el('td',fmt(payouts.reduce((n,o)=>n+o.value,0),8)+' XID'),el('td',age(b.time)));return tr}));if(revision!==blockRevision)return;$('blockRows').replaceChildren(...rows);if(!rows.length)$('blockRows').innerHTML='<tr><td colspan="4" class="empty">No verified blocks available.</td></tr>'}
...
async function renderWallet(){
 ...
 for(const [label,addr,removable] of rows){
  ...
  row.append(el('strong',b?xcfFmt(b.spendable_sats)+' XID':'—'));
...
function requestQR(){const t=amountText($('rvAmount').value),a=t.amount||'';let err=t.error;if(!err&&a&&!(/^\d{0,9}(\.\d{0,8})?$/.test(a)&&Number(a)>0&&Number(a)<=1e8))err='Amount: above 0, at most 100,000,000, up to 8 decimals, with a dot (e.g. 1.5).';rvSeq++;if(err){showQR({error:err});return}send('walletQR',{address:rvAddress,amount:a,seq:rvSeq})}
...
function showQR(m){const img=$('rvImg'),note=$('rvNote');note.classList.toggle('err',!!m.error);$('rvQr').hidden=!!m.error;if(m.error){img.hidden=true;$('rvUri').replaceChildren(el('span','—'));note.textContent=m.error;return}
 img.src=m.png;img.style.width=img.style.height=(m.modules*4)+'px';img.hidden=false;$('rvUri').replaceChildren(addrCopyLine(m.uri,'Copy the payment request'));note.textContent='Scan with an xCoin wallet, or share the request. '+(profile.network==='mainnet'?'Mainnet XID only.':'Testnet A coins only.')}
